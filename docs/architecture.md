...
One narrow active-only exception covers the window where the pipeline's own fix commits are not yet objects in the crew worktree, so no code-identity match is possible: a same-branch run is also attributed when `active_steps` reports a running or fixing step with non-quiet recent activity and `branch_sync` reports `pipeline_owned`.
Custody is read first from the `branch_sync` object `axi status` itself returns, which costs no extra process and no network read; a status answer that omits `branch_sync` falls back to one bounded read-only `axi sync --check`, which does read the remote and which the CLI refuses on an unclean worktree, in which case custody stays unproven and the exception simply does not fire.
Terminal, cancelled, parked, quiet, and activity-unknown runs still require the ordinary code-identity match, so a stale historical run on a reused branch is never attributed on branch custody alone.
The script header owns the exact run-head ancestry rules and that exception's precise conditions.
