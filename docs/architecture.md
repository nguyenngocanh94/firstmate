...
One narrow active-only exception covers the window where the pipeline's own fix commits are not yet objects in the crew worktree, so no code-identity match is possible: a same-branch run is also attributed when `active_steps` reports a running or fixing step with non-quiet recent activity and `branch_sync` reports `pipeline_owned`, read from the object `axi status` already returned and falling back to a read-only `axi sync --check` only when status omits it.
