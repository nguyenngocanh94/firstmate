...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run, active or terminal, only when it matches the crew's branch and, as the general rule, the crew's current code identity, then keeps that run-step authoritative even if the pane has closed.
One narrow active-only exception covers the window where the pipeline's own fix commits are not yet objects in the crew worktree, so no code-identity match is possible: a same-branch run is also attributed when `active_steps` reports a running or fixing step with non-quiet recent activity.
That local active-and-recent signal alone is sufficient; branch custody is never required, nothing is re-read from the remote, and the whole check reuses the `axi status` answer already in hand, so the heartbeat path gains no network read.
Terminal, cancelled, parked, quiet, and activity-unknown runs still require the ordinary code-identity match, so a stale historical run on a reused branch is never attributed on a branch-name match alone.
