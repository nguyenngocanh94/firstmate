...
A fifth pins the page framing flags the board depends on - the page title, a relative back-link, and the self-reload meta - including that an absolute or off-host back-link is refused and writes no page, and that a page rendered without them carries no reload and still no scripts.
...
It also pins the cost bound and the live/finished split directly: a running task's page carries the meta refresh and is re-rendered each tick, while a finished task's page carries none and is not rewritten while its report is unchanged.
