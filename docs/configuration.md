...
agy is empirically verified for crewmate and scout launches only, and a named agy secondmate spawn is refused before any endpoint is created.
...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when axi status answers but cannot be attributed to the crew
