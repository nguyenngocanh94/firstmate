...
## grok (VERIFIED 2026-06-29, grok 0.2.73; slash-submit re-verified 2026-07-03 on 0.2.82; reasoning-effort ceiling re-verified 2026-07-13 on 0.2.99; exit paths re-verified 2026-07-19 on grok 0.2.103; busy state and interrupt re-verified 2026-08-24 on grok 1.0.5)
...
| Fact | Value |
|---|---|
| Busy state | The one remaining rendered-tail fallback, isolated to Grok until its structured lifecycle is live-verified. On 1.0.x there are TWO independent mid-turn signals and either alone is sufficient: `Esc:cancel` in the keybind bar, and a bracketed `[stop]` ending the live status row. The working bar reads `Shift+Tab:mode │ Esc:cancel │ Ctrl+.:shortcuts`; every idle shape drops that cancel entry - a bare never-prompted launch shows no keybind bar at all, a launch whose prompt is still queued shows `Ctrl+;:queue`, a settled turn shows `Shift+Tab:mode │ Ctrl+.:shortcuts`, and a cancelled turn shows `Enter:send`. `Ctrl+c:cancel` is grok 0.2.x's bar, retained for that family only; 1.0.5 never prints it, which is why a stale signature read every working 1.0.x worker as idle. ASCII tokens are matched rather than the braille spinner to avoid locale fragility, and the `<label>… <elapsed>s` spinner shape is deliberately NOT a signal because it goes dark for the whole duration of a tool call. `bin/fm-busy-lib.sh` owns the expression and `bin/fm-tmux-lib.sh` derives the delivery guard from it. |
...
| Interrupt | On 1.0.5 a single `Ctrl+C` AND a single `Esc` each cancel the running turn, and each then RESTORES the cancelled prompt into the composer as unsubmitted text, leaving the bar at `Enter:send │ Shift+Tab:mode │ Ctrl+.:shortcuts`. So an interrupted grok pane reads idle with PENDING composer content, and the next thing typed appends to the restored text instead of starting clean - clear the composer before steering an interrupted grok worker. On 0.2.x `Ctrl+C` was the interrupt and `Esc` only moved focus to the scrollback; that no longer holds, so never send `Esc` to a grok pane expecting it to be inert. |
...
As of grok 1.0.5 (observed 2026-08-24) an empty composer renders as a bare `❯` with no placeholder text at all, so the `Type a message...` idle-placeholder override in the herdr, orca, and cmux adapters now matches nothing for grok.
It is harmless rather than stale-dangerous - the shared classifier already reads a bare `❯` as an empty agent composer - and the ghost stripper stays needed for the harnesses that do render de-emphasised placeholders.
